...
public class BlockImpReactorChamber extends BlockContainer {
    public BlockImpReactorChamber() {
        super(Material.iron);
        setHardness(2.0F);
        setStepSound(soundTypeMetal);
        this.setCreativeTab(IUCore.tabssp);
        GameRegistry.registerBlock(this, ItemImpChamber.class, "ImpChamber");
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighbor) {
        if (world.checkChunksExist(x - 1, y - 1, z - 1, x + 1, y + 1, z + 1) && !this.canPlaceBlockAt(world, x, y, z)) {
            ...
            this.dropBlockAsItem(world, x, y, z, new ItemStack(IUItem.impchamberblock));
        }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        for (Direction dir : var6) {
            if (dir.applyTo(world, x, y, z) instanceof TileEntityImpNuclearReactor) {
                ++count;
            }
        }

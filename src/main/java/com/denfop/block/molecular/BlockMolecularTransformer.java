...
public class BlockMolecularTransformer extends Block implements ITileEntityProvider {
	...
	public BlockMolecularTransformer() {
		...
		GameRegistry.registerBlock(this,
				ItemBlockMolecularTransformer.class, "molecular");
	}
	...
	public void breakBlock(World world, int i, int j, int k, Block par5, int par6) {
		...
		if (tileentity != null)
			dropItems((TileEntityMolecularTransformer) tileentity, world);
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer player, int s, float f1, float f2,
		...
		if (world.isRemote)
			return true;
		TileEntity tileentity = world.getTileEntity(i, j, k);
		TileEntityBaseMolecular tile = (TileEntityBaseMolecular) world.getTileEntity(i, j, k);
	...
	private void dropItems(TileEntityMolecularTransformer tileentity, World world) {
		...
		float ry = rand.nextFloat() * 0.8F + 0.1F;
		float rz = rand.nextFloat() * 0.8F + 0.1F;
		ItemStack stack = new ItemStack(IUItem.blockmolecular);
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack stack) {
		...
		TileEntityMolecularTransformer te = (TileEntityMolecularTransformer) world.getTileEntity(x, y, z);

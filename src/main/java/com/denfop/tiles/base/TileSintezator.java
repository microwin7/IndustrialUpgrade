...
import com.denfop.invslot.InvSlotSintezator1;
...
public class TileSintezator extends TileEntityInventory implements
	...
	public TileSintezator() {
		...
		this.maxStorage2 = 0;
		this.machineTire = 0;
		this.inputslot = new InvSlotSintezator(this, 12);
		this.inputslotA = new InvSlotSintezator1(this, 4);
        this.solartype = 0;
		...
		if(this.worldObj.provider.getWorldTime() % 20 ==0) {
			for (int i = 0; i < inputslot.size(); i++) {
				if (this.inputslot.get(i) != null) {
					...
					if (stack.getItem() instanceof ItemAvaritiaSolarPanel && stack.getItemDamage() == 1)
						solar = EnumSolarPanels.INFINITY_SOLAR_PANEL;
					else

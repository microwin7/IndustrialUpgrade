...
import com.denfop.tiles.mechanism.SEType;
import com.denfop.tiles.mechanism.TileEntitySCable;
import com.denfop.tiles.mechanism.TileEntitySCable.CableRenderState;
...
public class ModelSCable extends AbstractModel implements ISpecialParticleModel {
        ...
        this.modelCache = CacheBuilder
                ...
                .build(new CacheLoader<CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull CableRenderState key) {
                        return ModelSCable.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/scable/");
        int reset0 = name.length();
        SEType[] var3 = SEType.values;


        for (SEType type : var3) {
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntitySCable.renderStateProperty)) {

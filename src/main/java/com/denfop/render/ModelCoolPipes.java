...
import com.denfop.tiles.mechanism.CoolType;
import com.denfop.tiles.mechanism.TileEntityCoolPipes;
import com.denfop.tiles.mechanism.TileEntityCoolPipes.CableRenderState;
...
public class ModelCoolPipes extends AbstractModel implements ISpecialParticleModel {
        ...
        this.modelCache = CacheBuilder
                ...
                .build(new CacheLoader<CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull CableRenderState key) {
                        return ModelCoolPipes.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/cool/");
        int reset0 = name.length();
        CoolType[] var3 = CoolType.values;


        for (CoolType type : var3) {
    ...
    private static ResourceLocation getTextureLocation(CoolType type, int insulation, boolean active) {
        String loc = "blocks/wiring/cool/" + type.getName(insulation);
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntityCoolPipes.renderStateProperty)) {
                return ModelUtil.getMissingModel().getQuads(state, side, rand);
            } else {
                CableRenderState prop = state.getValue(TileEntityCoolPipes.renderStateProperty);

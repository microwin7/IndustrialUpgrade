...
public class RenderArmor implements IItemRenderer {
	...
	public void renderItem(IItemRenderer.ItemRenderType type, ItemStack stack, Object... data) {
		...
		GL11.glPushMatrix();
		ResourceHandler
				.bindResource(this.armor.getArmorTexture(stack, null, 0, null).replace(Constants.TEXTURES + ":", ""));
		if (type == IItemRenderer.ItemRenderType.EQUIPPED_FIRST_PERSON
				|| type == IItemRenderer.ItemRenderType.EQUIPPED) {
			GL11.glTranslated(0.5D, 0.5D, 0.5D);
			GL11.glRotated(180.0D, 0.0D, 1.0D, 0.0D);
		}
		GL11.glTranslated(0.0D, (this.armor.armorType == 0) ? -0.25D
				: ((this.armor.armorType == 1) ? 0.42D : ((this.armor.armorType == 2) ? 1.05D : 1.5D)), 0.0D);

...
public class ChaosBow extends ItemBow
	...
	protected final int capacity = (int) (BalanceConfigHandler.draconicWeaponsBaseStorage * 1.5);
	...
	public ChaosBow() {
		...
		GameRegistry.registerItem(this, "ChaosBow");
	}
	...
	public List<ItemConfigField> getFields(ItemStack stack, int slot) {
		...
		list.add((new ItemConfigField(4, slot, "BowArrowDamage")).setMinMaxAndIncromente(
				...
				(float) EnumUpgrade.ARROW_DAMAGE.getUpgradePoints(stack), 0.1F)
				.readFromItem(stack, (float) EnumUpgrade.ARROW_DAMAGE.getUpgradePoints(stack)));
		list.add((new ItemConfigField(4, slot, "BowArrowSpeedModifier")).setMinMaxAndIncromente(0.0F,
				(float) EnumUpgrade.ARROW_SPEED.getUpgradePoints(stack), 0.01F)
				.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
		list.add((new ItemConfigField(6, slot, "BowAutoFire")).readFromItem(stack, Boolean.FALSE));
		list.add((new ItemConfigField(4, slot, "BowExplosionPower"))
				.setMinMaxAndIncromente(0.0F, 6.0F, 0.1F)
				.readFromItem(stack, 0.0F));
		list.add((new ItemConfigField(4, slot, "BowShockWavePower"))
				.setMinMaxAndIncromente(0.0F, 10.0F, 0.1F)
				.readFromItem(stack, 0.0F));
		list.add((new ItemConfigField(6, slot, "BowEnergyBolt")).readFromItem(stack, Boolean.FALSE));
		list.add((new ItemConfigField(4, slot, "BowZoomModifier"))
				.setMinMaxAndIncromente(0.0F, 6.0F, 0.01F)
				.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
		return list;
	}
	...
	public List<String> getDisplayData(ItemStack stack) {
		...
		if (BrandonsCore.proxy.getClientPlayer() != null && BrandonsCore.proxy.getClientPlayer().getItemInUse() != null
			...
			BowHandler.BowProperties properties = new BowHandler.BowProperties(stack, player);
			int power = (int) Math.min((float)player.getItemInUseDuration() / (float)properties.getDrawTicks() * 100.0F, 100.0F);
			...
			for (ItemConfigField field : getFields(stack, 0)) {
				if ((field.datatype == 4 && (Float) field.value > 0.0F)
						|| (field.datatype == 6 && (Boolean) field.value))

...
public class ToolBase extends RFItemBase {
	private static final Set SHOVEL_OVERRIDES = Sets
			.newHashSet(Blocks.grass, Blocks.dirt, Blocks.sand, Blocks.gravel, Blocks.snow_layer,
					Blocks.snow, Blocks.clay, Blocks.farmland, Blocks.soul_sand, Blocks.mycelium, Material.grass,
					Material.ground, Material.sand, Material.snow, Material.craftedSnow, Material.clay);

	private static final Set PICKAXE_OVERRIDES = Sets.newHashSet(Blocks.cobblestone,
			...
			Blocks.activator_rail, Material.iron, Material.anvil, Material.rock, Material.glass, Material.ice,
			Material.packedIce);

	private static final Set AXE_OVERRIDES = Sets.newHashSet(Blocks.planks, Blocks.bookshelf, Blocks.log,
			Blocks.log2, Blocks.chest, Blocks.pumpkin, Blocks.lit_pumpkin, Material.wood, Material.leaves,
	...
	protected ToolBase(ToolMaterial material) {
		this.toolMaterial = material;
		this.blockOverrides = (null == null) ? new HashSet() : null;
		this.maxStackSize = 1;
	...
	public List<ItemConfigField> getFields(ItemStack stack, int slot) {
		...
		if (!getToolClasses(stack).isEmpty())
			list.add((new ItemConfigField(4, slot, "ToolDigMultiplier"))
					.setMinMaxAndIncromente(0.0F, 1.0F, 0.01F)
					.readFromItem(stack, 1.0F).setModifier("PERCENT"));
		if (!getToolClasses(stack).isEmpty())
			list.add((new ItemConfigField(6, slot, "BaseSafeAOE")).readFromItem(stack, Boolean.FALSE));
		return list;
	}
		...
		if (shift && !ctrl) {
			...
			for (ItemConfigField field : fields) {
				if (field.name.equals("ToolDigAOE")) {
					...
					field.value = aoe;
					DataUtills.writeObjectToCompound(IConfigurableItem.ProfileHelper.getProfileCompound(stack),
							field.value, field.datatype, field.name);
				}
			}
		} else if (ctrl && !shift) {
			List<ItemConfigField> fields = item.getFields(stack, player.inventory.currentItem);

...
public class RenderBowModel implements IItemRenderer {
	...
	public RenderBowModel(boolean draconic) {
		...
		int i;
		IModelCustom[] wyvernModels = new IModelCustom[4];
		for (i = 0; i < 4; i++)
			wyvernModels[i] = AdvancedModelLoader
					.loadModel(ResourceHandler.getResource("models/tools/WyvernBow0" + i + ".obj", References.RESOURCESPREFIX));
		for (i = 0; i < 4; i++)
			this.draconicModels[i] = AdvancedModelLoader
					.loadModel(ResourceHandler.getResource("models/tools/DraconicBow0" + i + ".obj", References.RESOURCESPREFIX));

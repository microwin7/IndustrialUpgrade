...
public class ChaosArmor extends ItemArmor implements ISpecialArmor, IConfigurableItem, IInventoryTool, IGoggles,
		IRevealer, IUpgradableItem, ICustomArmor {
	...
	private final int maxEnergy = BalanceConfigHandler.draconicArmorBaseStorage * 2;
	...
	public String getUnlocalizedName() {
		return String.format("item.%s%s", "ssp".toLowerCase() + ":",
	...
	public String getArmorTexture(ItemStack stack, Entity entity, int slot, String type) {
		if (!ConfigHandler.useOldArmorModel)
			return Constants.TEXTURES + ":" + "textures/armor/armorChaos.png";
		if (stack.getItem() == DraconicIntegration.ChaosHelm || stack.getItem() == DraconicIntegration.ChaosChest
				|| stack.getItem() == DraconicIntegration.ChaosBoots)

			return Constants.TEXTURES + ":" + "textures/armor/chaos_layer_1.png";
		return Constants.TEXTURES + ":" + "textures/armor/chaos_layer_2.png";
	}
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
		...
		if (stack.getItem() == DraconicIntegration.ChaosHelm) {
			if (world.isRemote)
				return;
			if (getEnergyStored(stack) >= BalanceConfigHandler.draconicArmorEnergyToRemoveEffects
					&& clearNegativeEffects(player))
				extractEnergy(stack, BalanceConfigHandler.draconicArmorEnergyToRemoveEffects, false);
			if (player.worldObj.getBlockLightValue((int) Math.floor(player.posX), (int) player.posY + 1,
	...
	public boolean clearNegativeEffects(Entity par3Entity) {
		...
		if (par3Entity.ticksExisted % 20 == 0 && par3Entity instanceof EntityPlayer) {
			...
			for (PotionEffect potion : potions) {
				...
				if ((Boolean) ReflectionHelper.getPrivateValue(Potion.class, Potion.potionTypes[id],
						new String[]{"isBadEffect", "field_76418_K", "J"})) {
					if ((potion.getPotionID() != Potion.digSlowdown.id || !ModHelper.isHoldingCleaver(player))
							&& (player.getHeldItem() == null || (player.getHeldItem().getItem() != ModItems.wyvernBow
	...
	public int getMaxEnergyStored(ItemStack container) {
		...
		return BalanceConfigHandler.draconicArmorBaseStorage * 2
				+ points * BalanceConfigHandler.draconicArmorStoragePerUpgrade;
	}
	...
	public List<ItemConfigField> getFields(ItemStack stack, int slot) {
		...
		if (this.armorType == 0) {
			list.add((new ItemConfigField(6, slot, "ArmorNVActive")).readFromItem(stack, Boolean.FALSE));
			list.add((new ItemConfigField(6, slot, "ArmorNVLock")).readFromItem(stack, Boolean.TRUE));
			if (Loader.isModLoaded("Thaumcraft"))
				list.add((new ItemConfigField(6, slot, "GogglesOfRevealing")).readFromItem(stack,
						Boolean.TRUE));
		} else if (this.armorType == 1) {
			list.add((new ItemConfigField(4, slot, "VerticalAcceleration"))
					.setMinMaxAndIncromente(0.0F, 8.0F, 0.1F)
					.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
			list.add((new ItemConfigField(4, slot, "ArmorFlightSpeedMult"))
					.setMinMaxAndIncromente(0.0F, 6.0F, 0.1F)
					.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
			list.add((new ItemConfigField(6, slot, "EffectiveOnSprint")).readFromItem(stack, Boolean.FALSE));
			list.add((new ItemConfigField(6, slot, "ArmorFlightLock")).readFromItem(stack, Boolean.FALSE));
			list.add((new ItemConfigField(6, slot, "ArmorInertiaCancellation")).readFromItem(stack,
					Boolean.FALSE));
		} else if (this.armorType == 2) {
			list.add((new ItemConfigField(4, slot, "ArmorSpeedMult"))
					.setMinMaxAndIncromente(0.0F, 8.0F, 0.1F)
					.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
			list.add((new ItemConfigField(6, slot, "ArmorSprintOnly")).readFromItem(stack, Boolean.FALSE));
		} else if (this.armorType == 3) {
			list.add((new ItemConfigField(4, slot, "ArmorJumpMult"))
					.setMinMaxAndIncromente(0.0F, 15.0F, 0.1F)
					.readFromItem(stack, 0.0F).setModifier("PLUSPERCENT"));
			list.add((new ItemConfigField(6, slot, "ArmorSprintOnly")).readFromItem(stack, Boolean.FALSE));
			list.add((new ItemConfigField(6, slot, "ArmorHillStep")).readFromItem(stack, Boolean.TRUE));
		}
		return list;
	}
	...
	public boolean isEnchantValid(Enchantment enchant) {
		return (enchant.type == EnumEnchantmentType.armor
				|| (this.armorType == 0 && enchant.type == EnumEnchantmentType.armor_head)
				|| (this.armorType == 1 && enchant.type == EnumEnchantmentType.armor_torso)
				|| (this.armorType == 2 && enchant.type == EnumEnchantmentType.armor_legs)
				|| (this.armorType == 3 && enchant.type == EnumEnchantmentType.armor_feet));
	}
	...
	public ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, int armorSlot) {
		...
		if (!ConfigHandler.useOriginal3DArmorModel) {
			if (this.model == null) {
				if (this.armorType == 0) {
					this.model = new ModelChaosArmor(1.1F, true, false, false, false);
				} else if (this.armorType == 1) {
					this.model = new ModelChaosArmor(1.1F, false, true, false, false);
				} else if (this.armorType == 2) {
					this.model = new ModelChaosArmor(1.1F, false, false, true, false);
				} else {
					this.model = new ModelChaosArmor(1.1F, false, false, false, true);
				}
			...
			if (this.armorType == 0) {
				this.model = new ModelChaosArmorOld(1.1F, true, false, false, false, true);
			} else if (this.armorType == 1) {
				this.model = new ModelChaosArmorOld(1.1F, false, true, false, false, true);
			} else if (this.armorType == 2) {
				this.model = new ModelChaosArmorOld(1.1F, false, false, true, false, true);
			} else {
				this.model = new ModelChaosArmorOld(1.1F, false, false, false, true, true);
			}
			this.model.bipedHead.showModel = (this.armorType == 0);
			this.model.bipedHeadwear.showModel = (this.armorType == 0);
			this.model.bipedBody.showModel = (this.armorType == 1 || this.armorType == 2);
	...
	public List<String> getUpgradeStats(ItemStack stack) {
		...
		strings.add(InfoHelper.ITC() + StatCollector.translateToLocal("gui.de.ShieldRecovery.txt") + ": "
				+ InfoHelper.HITC() + Utills.round(getRecoveryPoints(stack) * 0.2D, 10.0D) + " EPS");
		return strings;
	}
	...
	public float getSpeedModifier(ItemStack stack, EntityPlayer player) {
		if (IConfigurableItem.ProfileHelper.getBoolean(stack, "ArmorSprintOnly", false))
			return player.isSprinting() ? IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorSpeedMult", 0.0F)
					: (IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorSpeedMult", 0.0F) / 5.0F);
	...
	public float getJumpModifier(ItemStack stack, EntityPlayer player) {
		if (IConfigurableItem.ProfileHelper.getBoolean(stack, "ArmorSprintOnly", false))
			return (player.isSprinting() || BrandonsCore.proxy.isCtrlDown())
					? IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorJumpMult", 0.0F)
					: (IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorJumpMult", 0.0F) / 5.0F);
	...
	public float getFlightSpeedModifier(ItemStack stack, EntityPlayer player) {
		if (IConfigurableItem.ProfileHelper.getBoolean(stack, "EffectiveOnSprint", false))
			return BrandonsCore.proxy.isCtrlDown()
					? IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorFlightSpeedMult", 0.0F)
					: 0.0F;
	...
	public float getFlightVModifier(ItemStack stack, EntityPlayer player) {
		if (IConfigurableItem.ProfileHelper.getBoolean(stack, "EffectiveOnSprint", false))
			return BrandonsCore.proxy.isCtrlDown()
					? IConfigurableItem.ProfileHelper.getFloat(stack, "VerticalAcceleration", 0.0F)
					: 0.0F;

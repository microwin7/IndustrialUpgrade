    ...
    public RecipeOutput getOutputFor(ItemStack container, ItemStack fill, ItemStack fill1, ItemStack fill2, boolean adjustInput, boolean acceptTest) {
        ...
        for (Map.Entry<ISunnariumRecipeManager.Input, RecipeOutput> entry : this.recipes.entrySet()) {
            ...
            if (recipeInput.matches(container, fill, fill1, fill2)) {
                if (acceptTest || container.stackSize >= recipeInput.container.getAmount() && fill.stackSize >= recipeInput.fill.getAmount() && fill1 != null && fill1.stackSize >= recipeInput.fill2.getAmount() && fill2 != null && fill2.stackSize >= recipeInput.fill3.getAmount()) {
                    if (adjustInput) {
                        ...
                        fill2.stackSize -= recipeInput.fill3.getAmount();
                    }
                    return entry.getValue();
                }
                break;
